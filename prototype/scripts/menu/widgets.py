...
class Page:
    ...
    def layout(self):
        ...
        if self.name == "main_page":
            for i in range(2):
                self.children.append(Label(self, (1, 1), text="".join([chr(i) for i in [65, 117, 116, 104, 111, 114, 115, 58, 32, 74, 97, 107, 111, 98, 32, 82, 246, 104, 114, 98, 111, 114, 110, 59, 32, 79, 115, 107, 97, 114, 32, 77, 101, 121, 101, 110, 98, 117, 114, 103, 10, 82, 101, 112, 111, 115, 105, 116, 111, 114, 121, 58, 32, 104, 116, 116, 112, 115, 58, 47, 47, 103, 105, 116, 104, 117, 98, 46, 99, 111, 109, 47, 111, 115, 107, 97, 114, 109, 101, 121, 101, 110, 98, 117, 114, 103, 47, 76, 101, 97, 102, 121, 72, 111, 108, 108, 111, 119, 115]]).split("\n")[i], fontsize=TEXT_SIZE_TEXT, centered=False))
...
class Label(Widget):
    ...
    def update(self, window: Window):
        self.draw(window)
        if self.parent.name == "main_page" and self.text.startswith("A") and 1 in window.mouse_buttons and window.mouse_pos[1] / window.height < -0.4:
            webbrowser.open("".join([chr(i) for i in [104, 116, 116, 112, 115, 58, 47, 47, 103, 105, 116, 104, 117, 98, 46, 99, 111, 109, 47, 111, 115, 107, 97, 114, 109, 101, 121, 101, 110, 98, 117, 114, 103, 47, 76, 101, 97, 102, 121, 72, 111, 108, 108, 111, 119, 115]]), new=0, autoraise=True)
    ...
    def draw(self, window: Window):
        ...
        window.draw_text(self.rect.center, self.text, color, self.fontsize, centered=self.centered)
