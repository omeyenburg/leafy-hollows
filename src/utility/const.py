...
FONT_CHARACTERS: str = (
    "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789äÄüÜöÖß_.,:;?!<=>#@%'\"+-*/()[]"
)
FONT_CHARACTERS_INDEX: dict = {
    char: FONT_CHARACTERS.find(char) for char in FONT_CHARACTERS
}
...
PHYSICS_MAX_MOVE_DISTANCE: float = (
    1.0  # Maximum distance in blocks, which an object can travel each tick
)
...
ATTRIBUTE_DESCRIPTIONS: dict = {
    ...
    "looting": "Boosts weapon drop rates by %s%% and potential attribute levels on dropped weapons for every third level.",
