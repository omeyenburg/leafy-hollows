...
def load():
    ...
    user_options = file.load(
        "data/user/options.json", default=default, file_format="json"
    )
    ...
    for keyword, value in user_options.items():
        ...
        if (
            (
                isinstance(options[keyword], (int, bool))
                and not isinstance(value, (int, bool))
            )
            or (
                isinstance(options[keyword], float)
                and not isinstance(value, (float, int, bool))
            )
            or (isinstance(options[keyword], str) and not isinstance(value, str))
        ):
