...
class Rect:
    def __init__(self, *args):
        ...
        if args_length != 1 or len(args[0]) != 4:
            raise TypeError(
                "rect expected 1 list, 2 list or 4 numeric arguments, got "
                + str(args_length)
            )
        ...
        raise TypeError(
            "argument must be list, tuple or rect, got '" + type(args) + "'"
        )
    ...
    def collide_point(self, point):
        return (
            self.x < point[0] < self.x + self.w and self.y < point[1] < self.y + self.h
        )

    def collide_line(self, start, end):
        return (
            self.x < start[0] < self.x + self.w
            and self.y < start[1] < self.y + self.h  # Start point in rect
            or self.x < end[0] < self.x + self.w
            and self.y < end[1] < self.y + self.h  # End point in rect
            or min(start[0], end[0]) < self.x < max(start[0], end[0])
            and self.y
            < (self.x - start[0]) / (end[0] - start[0]) * (end[1] - start[1]) + start[1]
            < self.y + self.h  # Line intersects with right edge
            or min(start[0], end[0]) < self.x + self.w < max(start[0], end[0])
            and self.y
            < (self.x + self.w - start[0]) / (end[0] - start[0]) * (end[1] - start[1])
            + start[1]
            < self.y + self.h  # Line intersects with left edge
            or min(start[1], end[1]) < self.y < max(start[1], end[1])
            and self.x
            < (self.y - start[1]) / (end[1] - start[1]) * (end[0] - start[0]) + start[0]
            < self.x + self.w  # Line intersects with top edge
        )
