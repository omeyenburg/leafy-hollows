...
class Page:
    ...
    def layout(self):
        ...
        for i, child in enumerate(self.children):
            width[child.column] = max(
                width[child.column],
                child.rect.w / child.columnspan - self.spacing * (child.columnspan - 1),
            )
        ...
        for i, child in enumerate(self.children):
            ...
            child.rect.centerx = (
                sum(width[: child.column + 1])
                - total_width / 2
                + self.spacing * (child.column + (child.columnspan - 1) / 2)
                + width[child.column] * (child.columnspan - 1) / 2
                - width[child.column] / 2
            )
            child.rect.centery = (
                sum(height[: child.row + 1])
                - total_height / 2
                + self.spacing * child.row
                - height[child.row]
                + child.rect.h
                - height[child.row] / 2
            )
        ...
        if self.name == "main_page":
            for i in range(2):
                ...
                self.children[-1].rect.centerx = -0.98
                self.children[-1].rect.centery = -0.85 - 0.1 * i
                self.children[-1].layout_moved = True
    ...
    def update(self, window: Window):
        ...
        for child in self.children:
            ...
            if child.rect.collide_point(mouse_pos):
                if (
                    child.hover_time > MENU_DESCRIPTION_HOVER_TIME
                    and not child.hover_callback is None
                ):
...
class Widget:
    def __init__(
        ...
        self.name = "widget"
        self.parent: Page = parent
        self.rect = geometry.Rect(
            0, 0, *size
        )  # Rect will be moved, when parent.layout is called
...
class Label(Widget):
    def __init__(self, *args, text: str = "", centered=True, **kwargs):
    ...
    def update(self, window: Window):
        self.draw(window)
        if (
            self.parent.name == "main_page"
            and self.text.startswith("A")
            and 1 in window.mouse_buttons
            and window.mouse_pos[1] / window.height < -0.4
        ):
            webbrowser.open(
                "".join(
                    [
    ...
    def draw(self, window: Window):
        ...
        window.draw_text(
            self.rect.center, self.text, color, self.fontsize, centered=self.centered
        )
...
class Button(Widget):
    def __init__(
        self, *args, text: str = "", callback=None, duration: float = 0.2, **kwargs
    ):
        super().__init__(*args, **kwargs)
        self.text = text
        ...
        self.clicked = 0
        self.duration = (
            duration  # When button pressed: self.clicked > 0 for [duration] seconds
        )
    ...
    def draw_clicked(self, window: Window):
        ...
        window.draw_text(
            (self.rect.centerx, self.rect.centery - offset),
            self.text,
            (255, 255, 255, 200),
            self.fontsize,
            centered=True,
...
class Slider(Widget):
    ...
    def update(self, window: Window):
        ...
        self.slider_rect.x = (
            self.rect.x + (self.rect.w - self.slider_rect.w) * self.value
        )
        self.slider_rect.y = self.rect.y

        if (
            self.rect.collide_point(
                (
                    window.mouse_pos[0] / window.width * 2,
                    window.mouse_pos[1] / window.height * 2,
                )
            )
            and window.mouse_buttons[0] == 1
        ):
            self.selected = True
        ...
        if self.selected:
            value = min(
                ...
                max(
                    0,
                    (window.mouse_pos[0] / window.width * 2 - self.rect.x)
                    / self.rect.w,
                ),
            )
    ...
    def draw(self, window: Window):
        ...
        window.draw_image(
            "slider",
            (self.slider_rect[0], self.slider_rect[1] - offset),
            self.slider_rect[2:],
        )
        window.draw_text(
            (self.rect.centerx, self.rect.centery - offset),
...
class Entry(Widget):
    ...
    def draw(self, window: Window):
        ...
        window.draw_text(
            self.rect.center, self.text, (0, 0, 0, 255), self.fontsize, centered=True
        )
...
class ScrollBox(Widget):
    ...
    def update(self, window: Window):
        ...
        if self.offset_length == 0:
            ...
            self.slider_y = -round(
                (self.start_offset - self.offset) / self.offset_length, 4
            )
        ...
        self.slider_rect.y = (
            self.rect.y
            + self.rect.h
            - self.slider_y * (self.rect.h - self.slider_rect.h)
            - self.slider_rect.h
        )

        if (
            self.slider_rect.collide_point(
                (
                    window.mouse_pos[0] / window.width * 2,
                    window.mouse_pos[1] / window.height * 2,
                )
        ...
        if self.slider_selected and self.offset_length != 0:
            self.slider_y = (
                -(window.mouse_pos[1] / window.height * 2 - self.rect.y) / self.rect.h
                + self.rect.h
            )
    ...
    def layout(self):
        Page.layout(self)
        self.start_offset = (
            self.children[0].rect.y
            + self.children[0].rect.h
            - self.rect.y
            - self.rect.h
            + self.spacing
        )
    ...
    def draw(self, window: Window):
        ...
        if self.offset_length != 0:
            ...
            h = (
                min(
                    self.slider_rect[3] + self.slider_rect[1],
...
def HoverBox(window: Window, rect: list, text: list):
    ...
    for text_snippet, fontsize, color in text:
        ...
        pos = (rect[0] + start[0] + x, rect[1] + start[1] + y)
        x_offset, y_offset = window.draw_text(
            pos, text_snippet, color, size=fontsize, wrap=rect[2] - 0.05
        )
        wrap = text_snippet[-1] == "\n"
        if wrap:
            y += y_offset
