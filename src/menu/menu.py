# -*- coding: utf-8 -*-
from utility.language import translate
from utility.thread import threaded
...
class Menu:
    def __init__(self, window: Window):
        ...
        Button(
            ...
            callback=lambda: self.set_state("load_world"),
        )
        button_main_settings = Button(
            self.main_page,
        ...
        Button(
            ...
            MENU_BUTTON_SIZE,
            text="Play",
            callback=button_pause_play_update,
            fontsize=TEXT_SIZE_BUTTON,
        )
        Button(
            self.pause_page,
            MENU_BUTTON_SIZE,
            text="Main Menu",
            callback=button_pause_menu_update,
            fontsize=TEXT_SIZE_BUTTON,
        )
        ...
        self.inventory_page.fusing = 0
        self.inventory_page.fuse_item = None
        self.inventory_page.secondary_fuse_item = None
        ...
        Button(
            ...
            MENU_BUTTON_SIZE,
            text="Main Menu",
            callback=button_death_menu_update,
            fontsize=TEXT_SIZE_BUTTON,
        )
        ...
        def update_loading_page():
            ...
            if self.window.loading_progress[2]:
                ...
                self.loading_page_progress = max(
                    0.05,
                    min(
                        1.0,
                        (
                            self.loading_page_progress
                            + self.window.loading_progress[1]
                            / self.window.loading_progress[2]
                        )
                        / 2,
                    ),
                )
        ...
        self.loading_page = Page(
            columns=1, spacing=MENU_SPACING, callback=update_loading_page
        )
        title_loading_page = Label(
            self.loading_page, MENU_TITLE_SIZE, fontsize=TEXT_SIZE_HEADING
        )
        progress_loading_page = LoadingBar(self.loading_page, MENU_BUTTON_SIZE)
        description_loading_page = Label(
            self.loading_page, MENU_BUTTON_SIZE, fontsize=TEXT_SIZE_BUTTON
        )
        ...
        button_settings_video_open = Button(
            settings_page,
            MENU_BUTTON_SMALL_SIZE,
            text="Video Settings",
            fontsize=TEXT_SIZE_BUTTON,
        )
        button_settings_audio_open = Button(
            settings_page,
            MENU_BUTTON_SMALL_SIZE,
            text="Audio Settings",
            fontsize=TEXT_SIZE_BUTTON,
        )
        button_settings_control_open = Button(
            settings_page,
            MENU_BUTTON_SMALL_SIZE,
            text="Control Settings",
            fontsize=TEXT_SIZE_BUTTON,
        )
        button_settings_world_open = Button(
            settings_page,
            MENU_BUTTON_SMALL_SIZE,
            text="World Settings",
            fontsize=TEXT_SIZE_BUTTON,
        ...
        ###---###  Video settings page  ###---###
        settings_video_page = Page(
            parent=settings_page, spacing=MENU_SPACING, columns=2
        )
        ...
        slider_text_resolution = Slider(
            settings_video_page, MENU_BUTTON_SMALL_SIZE, value=value
        )
        ...
        slider_text_resolution.hover_callback = lambda: self.info_hover_box(
            slider_text_resolution.rect.centerx < 0, "Text Resolution", "medium"
        )
        ...
        if window.options["antialiasing"]:
            value = [i / 5 for i in range(1, 6)][
                round(log2(window.options["antialiasing"]))
            ]
        else:
            value = 0
        slider_antialiasing = Slider(
            settings_video_page, MENU_BUTTON_SMALL_SIZE, value=value
        )
        ...
        def slider_shadow_resolution_update():
            shadow_resolution = (0, 1, 2, 4, 8, 16, 32)[
                round(slider_shadow_resolution.value * 6)
            ]
            if shadow_resolution == 0:
                slider_shadow_resolution.text = "Shadow Resolution: Off"
            else:
                slider_shadow_resolution.text = (
                    "Shadow Resolution: " + f"{shadow_resolution:5d}"
                )
            window.options["shadow resolution"] = shadow_resolution
            window._instance_shader.setvar(
                "shadow_resolution", window.options["shadow resolution"]
            )

        if window.options["shadow resolution"]:
            value = [i / 6 for i in range(1, 7)][
                round(log2(window.options["shadow resolution"]))
            ]
        else:
            value = 0
        slider_shadow_resolution = Slider(
            settings_video_page, MENU_BUTTON_SMALL_SIZE, value=value
        )
        ...
        if PLATFORM != "Darwin":
            if PLATFORM == "Darwin":
                button_fullscreen = Button(
                    settings_video_page,
                    MENU_BUTTON_SMALL_SIZE,
                    callback=button_fullscreen_update,
                    text="Fullscreen: Disabled",
                    fontsize=TEXT_SIZE_OPTION,
                )
            else:
                button_fullscreen = Button(
                    settings_video_page,
            ...
            button_fullscreen.hover_callback = lambda: self.info_hover_box(
                button_fullscreen.rect.centerx < 0, "Fullscreen", "none"
            )

        # Reduce camera movement button
        def button_reduce_movement_update():
            window.options["reduce camera movement"] = not window.options[
                "reduce camera movement"
            ]
            button_reduce_movement.text = (
                "Reduce Movement: "
                + f"{str(window.options['reduce camera movement']):5}"
            )

        button_reduce_movement = Button(
            settings_video_page,
            MENU_BUTTON_SMALL_SIZE,
            callback=button_reduce_movement_update,
            text="Reduce Movement: " + str(window.options["reduce camera movement"]),
            fontsize=TEXT_SIZE_OPTION,
        )
        ...
        def button_show_fps_update():
            ...
            button_show_fps.text = (
                "Display Fps: " + f"{str(window.options['show fps']):5}"
            )

        button_show_fps = Button(
            settings_video_page,
            MENU_BUTTON_SMALL_SIZE,
            callback=button_show_fps_update,
            text="Display Fps: " + str(window.options["show fps"]),
            fontsize=TEXT_SIZE_OPTION,
        )
        button_show_fps.hover_callback = lambda: self.info_hover_box(
            button_show_fps.rect.centerx < 0, "Display Fps", "none"
        )

        # Show debug button
        def button_show_debug_update():
            window.options["show debug"] = not window.options["show debug"]
            button_show_debug.text = (
                "Display debug: " + f"{str(window.options['show debug']):5}"
            )

        button_show_debug = Button(
            settings_video_page,
            MENU_BUTTON_SMALL_SIZE,
            callback=button_show_debug_update,
            text="Display debug: " + str(window.options["show debug"]),
            fontsize=TEXT_SIZE_OPTION,
        )
        ...
        button_language = Button(
            settings_video_page,
            MENU_BUTTON_SMALL_SIZE,
            callback=button_language_update,
            text="Language: " + window.options["language"].title(),
            fontsize=TEXT_SIZE_OPTION,
        )
        ...
        ###---###  Audio settings page  ###---###
        settings_audio_page = Page(
            parent=settings_page, columns=2, spacing=MENU_SPACING
        )
        ...
        for category in filter(
            lambda option: option.endswith(" volume"), window.options
        ):
            value = window.options[category]
            if category == "master volume":
                slider_volume = Slider(
                    settings_audio_page, MENU_BUTTON_SIZE, columnspan=2, value=value
                )
            else:
                slider_volume = Slider(
                    settings_audio_page, MENU_BUTTON_SMALL_SIZE, value=value
                )
            slider_volume.callback = (
                lambda s=slider_volume, c=category: slider_volume_update(s, c)
            )
            slider_volume.callback()
        ...
        def slider_simulation_distance_update():
            ...
            slider_simulation_distance.text = (
                "Simulation Distance: " + f"{simulation_distance:2d}"
            )
            window.options["simulation distance"] = simulation_distance

        value = (window.options["simulation distance"] - 5) / 15
        slider_simulation_distance = Slider(
            settings_world_page, MENU_BUTTON_SMALL_SIZE, value=value
        )
        ...
        def button_auto_jump_update():
            ...
            button_auto_jump.text = (
                "Auto jump: " + f"{str(window.options['auto jump']):5}"
            )

        button_auto_jump = Button(
            settings_world_page,
            MENU_BUTTON_SMALL_SIZE,
            callback=button_auto_jump_update,
            text="Auto jump: " + str(window.options["auto jump"]),
            fontsize=TEXT_SIZE_OPTION,
        )
        ...
        def button_save_world_update():
            ...
            button_save_world.text = (
                "Save world: " + f"{str(window.options['save world']):5}"
            )

        button_save_world = Button(
            settings_world_page,
            MENU_BUTTON_SMALL_SIZE,
            callback=button_save_world_update,
            text="Save world: " + str(window.options["save world"]),
            fontsize=TEXT_SIZE_OPTION,
        )
        ...
        button_delete_world = Button(
            settings_world_page,
            MENU_BUTTON_SMALL_SIZE,
            callback=button_delete_world_update,
            text="Delete World...",
            fontsize=TEXT_SIZE_OPTION,
        )
        ...
        def button_delete_inventory_update():
            if file.exists("data/user/world.data") or file.exists(
                "data/user/inventory.data"
            ):
                delete_inventory_page.open()

        button_inventory_world = Button(
            settings_world_page,
            MENU_BUTTON_SMALL_SIZE,
            callback=button_delete_inventory_update,
        ...
        delete_world_page = Page(
            parent=settings_world_page, columns=1, spacing=MENU_SPACING
        )
        Label(
            delete_world_page,
            MENU_HEADING_SIZE,
            text="A new world will be generated. You will keep all aquired weapons.",
            fontsize=TEXT_SIZE_BUTTON,
        )
        Button(
            delete_world_page,
            MENU_BUTTON_SIZE,
            callback=button_delete_world_confirm_update,
            text="Delete my world",
            fontsize=TEXT_SIZE_BUTTON,
        )
        Button(
            delete_world_page,
            MENU_BUTTON_SIZE,
            callback=settings_world_page.open,
            text="Cancel",
            fontsize=TEXT_SIZE_BUTTON,
        )
        ...
        delete_inventory_page = Page(
            parent=settings_world_page, columns=1, spacing=MENU_SPACING
        )
        Label(
            delete_inventory_page,
            MENU_HEADING_SIZE,
            text="This will reset your inventory and generate a new world.",
            fontsize=TEXT_SIZE_BUTTON,
        )
        Button(
            delete_inventory_page,
            MENU_BUTTON_SIZE,
            callback=button_delete_inventory_confirm_update,
            text="Reset progress",
            fontsize=TEXT_SIZE_BUTTON,
        )
        Button(
        ...
        settings_control_page = Page(
            parent=settings_page, columns=2, spacing=MENU_SPACING
        )
        Label(settings_control_page, MENU_BUTTON_SMALL_SIZE)
        Label(
            settings_control_page,
            MENU_HEADING_SIZE,
            columnspan=2,
            text="Click a button and press a key to bind a new key to an action",
            fontsize=TEXT_SIZE_TEXT,
        )

        scrollbox = ScrollBox(
            settings_control_page,
            (MENU_BUTTON_WIDTH + MENU_SPACING, MENU_SCROLL_BOX_HEIGHT),
            columnspan=2,
            columns=2,
        ...
        def update_key():
            ...
            if scrollbox.selected and not selected is None:
                keys = (
                    window.get_pressed_mods()
                    + window.get_pressed_keys()
                    + window.get_pressed_mouse()
                )
                ...
                if keys:
                    ...
                    window.keys: dict = dict.fromkeys(
                        [
                            value
        ...
        for i, key in enumerate(keys):
            ...
            buttons[key] = Button(
                ...
                row=i,
                column=1,
                callback=lambda key=key: select_key(key),
                text=window.options[key].title(),
                duration=-1,
                fontsize=TEXT_SIZE_BUTTON,
            )
    ...
    def get_intro_texts(self):
        ...
        formatter = (
            (),
            (
                self.window.options["key.left"].title(),
                self.window.options["key.right"].title(),
            ),
        ...
        return [
            self.translate(text) % value for text, value in zip(intro_texts, formatter)
        ]

    def info_hover_box(
        self, side, title="", performance_impact="", description="", height=1
    ):
        ...
        rect = [
            0,
            -height / 2 + MENU_DESCRIPTION_BOX_Y,
            MENU_BUTTON_SMALL_WIDTH + MENU_SPACING,
            MENU_DESCRIPTION_BOX_HEIGHT * height,
        ]
        ...
        while not finished:
            ...
            if self.window.options["show fps"]:
                self.window.draw_text(
                    (-0.98, 0.95),
                    f"FPS: {self.window.average_fps: 8.3F}",
                    (250, 250, 250, 200),
