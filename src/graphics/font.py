...
class Font:
    def __new__(cls, name=None, resolution=1, bold=False, antialias=False):
        ...
        if CREATE_TEXTURE_ATLAS_FILE:
            pygame.image.save(image, file.abspath("data/debug_font.png"))

        if ".ttf" in name:
            file.save(
                "data/fonts/" + name.replace(".ttf", ".json"),
                instance.char_rects,
                file_format="json",
            )
            pygame.image.save(
                image, file.abspath("data/fonts/" + name.replace(".ttf", ".png"))
            )
    ...
    def _load(self, name, resolution, bold, antialias):
        ...
        if ".json" in name:
            ...
            image = pygame.image.load(
                file.abspath("data/fonts/" + name.replace(".json", ".png"))
            )
            return image
        ...
        character_images = [
            font.render(char, antialias, (255, 255, 255)) for char in FONT_CHARACTERS
        ]

        # Get maximum character size
        self.char_width = max([char_img.get_width() for char_img in character_images])
        self.char_height = font.render(
            FONT_CHARACTERS, antialias, (0, 0, 0)
        ).get_height()
        ...
        for i in range(FONT_CHARACTERS_LENGTH):
            ...
            char_rect = (
                column * self.char_width / font_size[0],
                1 - row * self.char_height / font_size[1] - 1 / self.rows,
                1 / self.columns,
                1 / self.rows,
            )
