...
from utility.const import PLATFORM
...
class Window:
    def __init__(self, caption, block_data, block_atlas_image):
        ...
        self.keys: dict = dict.fromkeys(
            [value for key, value in self.options.items() if key.startswith("key.")], 0
        )  # 0 = Not pressed | 1 = Got pressed | 2 = Is pressed
        self.unicode: str = ""  # Backspace = "\x08"
        self.mouse_buttons: [int] = [
            0,
            0,
            0,
        ...
        self._font = pygame.font.Font(
            file.abspath("data/fonts/" + self._font_options[0]),
            self.options["text resolution"] * 5,
        )
    ...
    def _centered_text(
        ...
        """
        source = pygame.transform.scale_by(
            self._font.render(text, 0, (255, 255, 255)), size
        )
    ...
    def _uncentered_text(
        ...
        self._window.blit(
            source, (position[0], position[1] - letter_size[1] * size * 0.5)
        )
        total_text_size = source.get_size()
        return (
            total_text_size[0] / self.width * 2,
            -3 * total_text_size[1] / self.height,
        )
    ...
    def draw_image(
        ...
        source = pygame.transform.rotate(
            pygame.transform.flip(pygame.transform.scale(source, dest_rect[2:]), *flip),
            angle,
        )
        self._window.blit(
            source,
            (dest_rect[0] - dest_rect[2] * 0.5, dest_rect[1] - dest_rect[3] * 0.5),
        )
    ...
    def draw_post_processing(self):
        ...
        for x_coord, y_coord in numpy.ndindex(self.world_view.shape[:2]):
            for layer in (2, 0, 1):
                ...
                if block != 0:
                    rect = self.camera.map_coord(
                        (
                            x_coord
                            + 1
                            + floor(
                                self.camera.pos[0]
                                - self.world_view.shape[0] / 2
                                - offset[0]
                            ),
                            y_coord
                            + floor(
                                self.camera.pos[1]
                                - self.world_view.shape[1] / 2
                                - offset[1]
                            ),
                            1,
                            1,
                    ...
                    rect = (x * 16, height - (y + 1) * 16 - height % 16, 16, 16)
                    source = pygame.Surface(rect[2:], pygame.SRCALPHA)
                    source.blit(self._texBlocks, (0, 0), rect)
                    source = pygame.transform.scale(
                        pygame.transform.flip(source, not block % 2, 0), dest_rect[2:]
                    )
