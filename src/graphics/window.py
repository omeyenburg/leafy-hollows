# -*- coding: utf-8 -*-
from graphics.image import load_sprites, get_sprite_rect
from utility.const import OPENGL_VERSION, PLATFORM
...
class Window:
    def __init__(self, caption, block_data, block_atlas_image):
        ...
        if PLATFORM == "Darwin":
            self._mod_names = {
                pygame.__dict__[identifier]: identifier[4:]
                .replace("_R", "Right ")
                .replace("_L", "Left ")
                .replace("_", "")
                .replace("META", "Cmd")
                ...
                if identifier.startswith("KMOD_")
                and not identifier[5:]
                in ("NONE", "CTRL", "SHIFT", "ALT", "GUI", "META")
            }
        else:
            self._mod_names = {
                pygame.__dict__[identifier]: identifier[4:]
        ...
        self._key_names = tuple(
            [
                pygame.__dict__[identifier]
                for identifier in pygame.__dict__.keys()
        ...
        self.width, self.height = self.size = self.pre_fullscreen = (
            int(info.current_w / 3 * 2),
            int(info.current_h / 5 * 3),
        )
        self.stencil_rect = ()
        ...
        GL.glBufferData(
            GL.GL_ARRAY_BUFFER, 0, self._source_or_color_vbo_array, GL.GL_STREAM_COPY
        )
        ...
        GL.glBufferData(
            GL.GL_ARRAY_BUFFER, 0, self._shape_transform_vbo_array, GL.GL_STREAM_COPY
        )
        GL.glVertexAttribPointer(4, 4, GL.GL_FLOAT, GL.GL_FALSE, 0, ctypes.c_void_p(0))
        GL.glVertexAttribDivisor(4, 1)

        # Create sprite texture
        self.sprites, self.sprite_rects, self.hand_positions, sprite_atlas_image = (
            load_sprites()
        )
    ...
    def setup(self):
        ...
        particle_data = file.load(
            file.abspath("data/particles/particles.json"), file_format="json"
        )
    ...
    def _add_vbo_instance(self, dest, source_or_color, shape_transform):
        ...
        if self._vbo_instances_length == self._vbo_instances_index:
            ...
            new_source_or_color_vbo_array[: len(self._source_or_color_vbo_array)] = (
                self._source_or_color_vbo_array
            )
            GL.glBindBuffer(GL.GL_ARRAY_BUFFER, self._source_or_color_vbo)
            GL.glBufferData(
                GL.GL_ARRAY_BUFFER,
                new_source_or_color_vbo_array.nbytes,
                new_source_or_color_vbo_array,
                GL.GL_STREAM_COPY,
            )
            ...
            new_shape_transform_vbo_array[: len(self._shape_transform_vbo_array)] = (
                self._shape_transform_vbo_array
            )
            GL.glBindBuffer(GL.GL_ARRAY_BUFFER, self._shape_transform_vbo)
            GL.glBufferData(
                GL.GL_ARRAY_BUFFER,
                new_shape_transform_vbo_array.nbytes,
                new_shape_transform_vbo_array,
                GL.GL_STREAM_COPY,
            )
    ...
    def get_pressed_mouse(self):
        ...
        """
        return [
            ("Left Click", "Middle Click", "Right Click")[i]
            for i, pressed in enumerate(self.mouse_buttons)
            if pressed == 1
        ]
    ...
    def resize(self):
        ...
        self._window = pygame.display.set_mode(
            ...
            vsync=self.options["enable vsync"],
        )
        self._window = pygame.display.set_mode(
            (self.width, self.height),
            flags=flags | pygame.OPENGL,
    ...
    def _events(self):
        ...
        self.unicode = ""
        self.mouse_buttons = [
            2 if value == 1 else value for value in self.mouse_buttons
        ]
        ...
        for event in events:
            if event.type == pygame.QUIT:
                ...
                self.mouse_wheel = [
                    self.mouse_wheel[0] + event.x,
                    self.mouse_wheel[1] + event.y,
                    event.x,
                    event.y,
                ]
    ...
    def _update_world(self, player_position=(0, 0)):
        ...
        offset = (
            self.camera.pos[0] % 1
            - (self.width / 2 / self.camera.pixels_per_meter) % 1
            + self.options["simulation distance"],
            self.camera.pos[1] % 1
            - (self.height / 2 / self.camera.pixels_per_meter) % 1
            + self.options["simulation distance"],
        )
    ...
    def _draw_shadows(self, offset, player_position):
        ...
        shadow_resolution = self.options["shadow resolution"]
        surface_size = (
            round(view.shape[0] * shadow_resolution),
            round(view.shape[1] * shadow_resolution),
        )
        ...
        center = (floor(self.camera.pos[0]), floor(self.camera.pos[1]))
        start = (
            center[0]
            - floor(self.width / 2 / self.camera.pixels_per_meter)
            - self.options["simulation distance"],
            center[1]
            - floor(self.height / 2 / self.camera.pixels_per_meter)
            - self.options["simulation distance"],
        )
        player_position = (
            player_position[0] - start[0] - 1.0,
            player_position[1] - start[1] - 1.0,
        )
        ...
        triangle_points = shadow.get_triangle_points(
            view, shadow.List(player_position), numpy.array(corners), shadow.List(edges)
        )
        ...
        triangle_points = tuple(
            map(
                lambda n: (
                    round(n[1] * shadow_resolution),
                    round(n[2] * shadow_resolution),
                ),
                triangle_points,
            )
        )
    ...
    def _centered_text(
        ...
        x_offset = 0
        x_factor_fixed = (
            1 / self.width * self.screen_size[0]
        )  # Used when fixed_size == 1
        y_factor_fixed = (
            1 / self.height * self.screen_size[1]
        )  # Used when fixed_size == 1
        y_factor_relational = (
            1 / self.height * self.screen_size[1] * self.width / self.screen_size[0]
        )  # Used when fixed_size == 2
        if len(color) == 3:
            color = (*color, 255)
        ...
        for letter in text:
            ...
            if fixed_size == 0:
                ...
                dest_rect = (
                    ...
                    position[1],
                    rect[2] * size,
                    rect[3] * 2 * size * y_factor_relational,
                )
                ...
                dest_rect = (
                    position[0] + x_offset + rect[2],
                    position[1],
                    rect[2] * size * x_factor_fixed,
                    rect[3] * 2 * size * y_factor_fixed,
                )
            ...
            if self.stencil_rect:
                ...
                if org[2] - width > 0.0001 or org[3] - height > 0.0001:
                    source_and_color = (
                        ...
                        + rect[0]
                        + rect[2]
                        * ((1 - dest_rect[2] / org[2]) if dest_rect[0] > org[0] else 0),
                        color[1]
                        + rect[1]
                        + rect[3]
                        * (
                            round(1 - dest_rect[3] / org[3], 6)
                            if dest_rect[1] > org[1]
                            else 0
                        ),
    ...
    def _uncentered_text(
        ...
        if not len(text.strip()):
            ...
            for i, letter in enumerate(text):
                ...
                if fixed_size == 0:
                    dest_rect = (
                        position[0] + x_offset,
                        position[1] - y_offset * rect[3] * line_height * size,
                        rect[2] * size,
                        rect[3] * 2 * size,
                    )
                    ...
                    dest_rect = (
                        position[0] + x_offset,
                        position[1]
                        - y_offset * rect[3] * line_height * size * y_factor_relational,
                        rect[2] * size,
                        rect[3] * 2 * size * y_factor_relational,
                    )
                    letter_x_offset = rect[2] * spacing * size * 2
                    ...
                    dest_rect = (
                        position[0] + x_offset,
                        position[1]
                        - y_offset * rect[3] * line_height * size * y_factor_fixed,
                        rect[2] * size * x_factor_fixed,
                        rect[3] * 2 * size * y_factor_fixed,
                    )
                ...
                if (
                    (not wrap is None)
                    and next_space >= 0
                    and x_offset + letter_x_offset * next_space > wrap
                ):
                    x_offset = 0
                    y_offset += 1
        ...
        if fixed_size == 0:
            ...
            return (
                x_offset,
                rect[3] * 2
                - y_offset * rect[3] * 2 * line_height * size * y_factor_relational,
            )
        elif fixed_size == 2:
            return (
                x_offset,
                rect[3] * 2
                - y_offset * rect[3] * 2 * line_height * size * y_factor_fixed,
            )
    ...
    def draw_image(
        ...
        if self.stencil_rect:
            ...
            left = max(
                dest_rect[0] - dest_rect[2], self.stencil_rect[0] - self.stencil_rect[2]
            )
            right = min(
                dest_rect[0] + dest_rect[2], self.stencil_rect[0] + self.stencil_rect[2]
            )
    ...
    def draw_circle(self, position: [float], radius: int, color: [int]):
        ...
        self._add_vbo_instance(
            (
                *position,
                radius / self.width * self.screen_size[1],
                radius / self.height * self.screen_size[1],
            ),
            self.camera.map_color(color),
            (2, 0, 0, 0),
        )
    ...
    def draw_line(self, start: [float], end: [float], width: int, color: [int]):
        ...
        size = (
            sqrt(((start[0] - end[0]) / 2) ** 2 + ((start[1] - end[1]) / 2) ** 2),
    ...
    def draw_text(
        ...
        if centered:
            ...
            return self._uncentered_text(
                position, text, color, size, spacing, fixed_size, wrap
            )
