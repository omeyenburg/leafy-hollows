...
class Camera:
    ...
    def update(self):
        ...
        xvel = round(
            (self.dest[0] - self.pos[0]) * self.window.delta_time * self.speed, 3
        )
        yvel = round(
            (self.dest[1] - self.pos[1]) * self.window.delta_time * self.speed, 3
        )
        ...
        if self.resolution_index < 1:
            self.resolution_index = (
                self.resolution_index
                + self.resolution_speed * self.window.delta_time * 60
            )
            factor = 1000 ** -(
                (self.resolution_index - 1) ** 2
            )  # f(x) = 1000^[-(x-1)^2]
            self.resolution = (
                self.resolution_start * (1 - factor) + self.resolution_goal * factor
            )
            self.pixels_per_meter = self.resolution * WORLD_BLOCK_SIZE

        self.shift_pos += (
            (self.shift_dest - self.shift_pos) * self.window.delta_time * 0.5
        )
        self.pos[0] += self.shift_pos

    def map_coord(
        self, coord: [float], from_world: bool = False, to_world: bool = False
    ):
        """
        ...
        if from_world:
            coord[0] = (
                (coord[0] - self.pos[0]) * self.pixels_per_meter / self.window.width * 2
            )
            coord[1] = (
                (coord[1] - self.pos[1])
    ...
    def visible_blocks(self):
        ...
        return (
            (
                center_x
                - floor(self.window.width / 2 / self.pixels_per_meter)
                - simulation_distance,
                center_y
                - floor(self.window.height / 2 / self.pixels_per_meter)
                - simulation_distance,
            ),
            (
                center_x
                + ceil(self.window.width / 2 / self.pixels_per_meter)
                + simulation_distance,
                center_y
                + ceil(self.window.height / 2 / self.pixels_per_meter)
                + simulation_distance,
            ),
        )
