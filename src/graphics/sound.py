...
def load():
    ...
    sound_files = {
        ...
        for path in (
            file.find("data/sounds", "*.wav", True)
            + file.find("data/sounds", "*.ogg", True)
        )
    }
    ...
    for path in sound_paths:
        ...
        files = tuple(
            [
                pygame.mixer.Sound(sound_files[sound_file])
                for sound_file in data["sounds"]
            ]
        )
...
def play(
    ...
    options_volume = (
        window.options[category + " volume"] * window.options["master volume"]
    )
