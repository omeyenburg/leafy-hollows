...
def spawn_possible(window, name):
    ...
    """
    return (
        window.options["particles"] and window.particle_types[name][0][0] < window.time
    )
...
def spawn(
    ...
    for _ in range(absolute_amount):
        ...
        window.particles.append(
            [
                name,
                x,
                y,
                window.time + window.particle_types[name][1],
                speed,
                angle + divergence * (random.random() - 0.5),
            ]
        )
...
def update_particle(window, i, particle):
    ...
    window.particles[i][2] += (
        sin(angle) * speed - window.particle_types[name][3]
    ) * window.delta_time
    if window.particle_types[name][7]:
        window.particles[i][5] += (
            (random.random() - 0.5 + window.particle_wind)
            * window.particle_types[name][7]
            / 200
        )
    size_factor = (
        1
        - (time - window.time)
        / window.particle_types[name][1]
        * window.particle_types[name][4]
    )
    size = (
        window.particle_types[name][8][0] * size_factor,
        window.particle_types[name][8][1] * size_factor,
    )

    rect = window.camera.map_coord(
        (x - size[0] / 2, y - size[1] / 2, *size), from_world=True
    )
...
def update_explosion(window, i, particle):
    ...
    image = "explosion_" + chr(
        ord("a") + round(6 - (time - window.time) / total_time * 6)
    )

    rect = window.camera.map_coord(
        (x - size / 2, y - size / 2, size, size), from_world=True
    )
