...
def get_sprite_rect(window, image, offset=0):
    ...
    if speed != 0:
        return window.sprite_rects[
            frames[int((window.time + 10 * offset) // speed % len(frames))]
        ]
...
def get_hand_position(window, image, offset=0):
    ...
    """
    if image not in window.hand_positions:
        return (0, 0, 0)
...
def load_blocks():
    ...
    for data in sorted(blocks, key=lambda data: data["hardness"]):
        ...
        if size == (WORLD_BLOCK_SIZE, WORLD_BLOCK_SIZE):
            ...
            block_group_size[block] = (
                size[0] // WORLD_BLOCK_SIZE,
                size[1] // WORLD_BLOCK_SIZE,
            )
            for frame in data["frames"]:
                for x in range(0, size[0], WORLD_BLOCK_SIZE):
                    for y in range(0, size[1], WORLD_BLOCK_SIZE):
                        ...
                        sub_block = (
                            f"{block}_{x // WORLD_BLOCK_SIZE}_{y // WORLD_BLOCK_SIZE}"
                        )
                        block_data[sub_block] = (
                            data["hardness"],
                            data["family"],
                            data["layer"],
                        )
                        animation.append(
                            (sub_block, len(data["frames"]), data["speed"])
                        )
    ...
    image = pygame.Surface(
        (width * WORLD_BLOCK_SIZE, height * WORLD_BLOCK_SIZE + block_animation_rows),
        pygame.SRCALPHA,
    )
    ...
    for i, frame in enumerate(frames):
        ...
        image.blit(
            block_surface,
            (x * WORLD_BLOCK_SIZE, (height - y - 1) * WORLD_BLOCK_SIZE),
            (*pos, WORLD_BLOCK_SIZE, WORLD_BLOCK_SIZE),
        )
    ...
    for block, length, speed in animation:
        image.set_at(
            (
                x % (width * WORLD_BLOCK_SIZE),
                height * WORLD_BLOCK_SIZE + x // (width * WORLD_BLOCK_SIZE),
            ),
            (length, speed * 255 / 2, families[block_data[block][1]]),
        )  # length: 0-255 | speed: 0.0-2.0
    ...
    if CREATE_TEXTURE_ATLAS_FILE:
        pygame.image.save(image, file.abspath("data/debug_blocks.png"))
...
def load_sprites():
    ...
    for image_data in images_data:
        ...
        rect = tuple(
            [float(x) for x in data.replace("(", "").replace(")", "").split(",")]
        )
    ...
    for image_path, i in paths.items():
        image.blit(
            pygame.image.load(image_path), (sprite_rects[i][0], sprite_rects[i][1])
        )
        sprite_rects[i] = (
            sprite_rects[i][0] / width,
            1 - sprite_rects[i][1] / height - sprite_rects[i][3] / height,
            sprite_rects[i][2] / width,
            sprite_rects[i][3] / height,
        )
    ...
    for path in sprite_paths:
        ...
        for frame in data["frames"]:
            ...
            try:
                ...
                raise Exception(
                    f"Could not find any data of '{frame}'.\nRun\n'python data/images/layout/setup.py'\nor\n'python3 data/images/layout/setup.py'"
                )
    ...
    if CREATE_TEXTURE_ATLAS_FILE:
        pygame.image.save(image, file.abspath("data/debug_sprites.png"))
