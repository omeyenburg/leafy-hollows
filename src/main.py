# -*- coding: utf-8 -*-
from graphics.image import load_blocks
...
def save_world():
    ...
    if (
        menu.game_state in ("game", "intro", "pause", "inventory", "fuse")
        and not world is None
    ):
...
def load_world():
    ...
    world = menu.load_threaded(
        "Loading world", "load_world", World.load, window, block_data, wait=True
    )
...
def draw_game():
    ...
    if window.options["show debug"]:
        ...
        window.draw_text(
            ...
            + str(
                (
                    round(world.player.rect.centerx, 1),
        ...
        window.draw_text(
            (-0.98, 0.75 - y_offset),
            "Mouse Pos: " + str((floor(mouse_pos[0]), floor(mouse_pos[1]))),
            (250, 250, 250, 200),
            size=TEXT_SIZE_DESCRIPTION,
        )
        window.draw_text(
        ...
        window.draw_text(
            ...
            + str(
                world.get_block(
                    floor(mouse_pos[0]), floor(mouse_pos[1]), layer=slice(None)
                )
            ),
            (250, 250, 250, 200),
            size=TEXT_SIZE_DESCRIPTION,
    ...
    if world.player.block_below or not (world.player.rect.x == 0):
        ...
        heart_width = 0.1
        heart_size = Vec(
            heart_width, heart_width / window.height * window.width / 7 * 6
        )
        health_bar_size = Vec(0.5, 0.05)

        window.draw_image("heart", heart_center - heart_size / 2, heart_size)
        window.draw_rect(
            heart_center + Vec(heart_size.x * 0.8, -health_bar_size.y / 2),
            (0.5 * health_percentage, 0.05),
            (165, 48, 48, 255),
        )
        window.draw_rect(
            heart_center
            + Vec(heart_size.x * 0.8 + 0.5 * health_percentage, -health_bar_size.y / 2),
            (0.5 * (1 - health_percentage), 0.05),
            (56, 29, 49, 255),
        )
        ...
        if weapon.image in ("bow", "banana"):
            arrow_text = (
                f"{world.player.inventory.arrows}/{world.player.inventory.max_arrows}"
            )
            window.draw_text((0.75, -0.42), arrow_text, (255, 255, 255), 0.17)
            window.draw_image(
                "arrow_item",
                (0.9, -0.5),
                (0.1, 0.1 / window.height * window.width),
                angle=0,
            )
        ...
        if world.player.recent_drop[0] > 0:
            ...
            if world.player.recent_drop[1] == "arrows":
                window.draw_text(
                    (0.8, drop_y + 0.05 / window.height * window.width),
                    "+" + str(world.player.recent_drop[2]),
                    (255, 255, 255),
                    0.25,
                )
                window.draw_image(
                ...
                window.draw_text(
                    (0.85, drop_y + 0.05 / window.height * window.width),
                    "+",
                    (255, 255, 255),
                    0.25,
                )
                window.draw_image(
                    world.player.recent_drop[1].image,
                    (0.9, drop_y),
                    (0.1, 0.1 / window.height * window.width),
                    angle=world.player.recent_drop[1].angle,
                )

        if window.camera.pos[0] >= world.camera_stop:
            window.draw_text(
                (0, 0.8),
                "You escaped from the caves!",
                (255, 255, 255),
                0.3,
                centered=True,
            )
...
def main():
    ...
    if window.options["test.edit_holding_enabled"]:
        ...
        if not len(animation_path):
            animation_path = file.find(
                "data/images/sprites", image + "_.json", True, True
            )
    ...
    while True:
        if menu.game_state == "game":
            ...
            if "+" in window.unicode:
                current_unique_frame_index = (current_unique_frame_index + 1) % len(
                    unique_frames
                )
            if "-" in window.unicode:
                current_unique_frame_index = (current_unique_frame_index - 1) % len(
            ...
            if "o" in window.unicode:
                ...
                for i, current_position in enumerate(pre_print_positions):
                    ...
                    next_position = pre_print_positions[
                        (i + 1) % len(pre_print_positions)
                    ]
                    ...
                    print_positions.append(
                        [interpolated_x, interpolated_y, interpolated_a]
                    )
            ...
            if unique_stepping:
                window.time = (
                    unique_frame_indices[current_unique_frame_index] + 0.01
                ) * speed
            ...
            if item_shown:
                rect = Rect(
                    *window.camera.map_coord(
                        (
                            item_positions[current_unique_frame_index][0] - 0.3,
                            item_positions[current_unique_frame_index][1] - 0.3,
                            0.6,
                            0.6,
                        ),
                        from_world=True,
                ...
                if (
                    unique_stepping
                    and rect.collide_point(
                        Vec(
                            window.mouse_pos[0] / window.width,
                            window.mouse_pos[1] / window.height,
                        )
                        * 2
                    )
                    and (
                        2 in window.mouse_buttons
                        or any(window.mouse_buttons)
                        and item_selected
                    )
                ):
                    item_selected = True
                    item_positions[current_unique_frame_index][0] += (
                        window.mouse_pos[2] / WORLD_BLOCK_SIZE / 2
                    )
                    item_positions[current_unique_frame_index][1] += (
                        window.mouse_pos[3] / WORLD_BLOCK_SIZE / 2
                    )
                else:
                    item_selected = False
                window.draw_image(
                    ...
                    rect[:2],
                    rect[2:],
                    flip=(not flip_direction, 0),
                    angle=item_positions[current_unique_frame_index][2],
                )
                if "q" in window.unicode:
                    item_positions[current_unique_frame_index][2] = (
                        item_positions[current_unique_frame_index][2] + 1
                    ) % 360
                ...
                if "Q" in window.unicode:
                    item_positions[current_unique_frame_index][2] = (
                        item_positions[current_unique_frame_index][2] + 10
                    ) % 360
                if "E" in window.unicode:
                    item_positions[current_unique_frame_index][2] = (
                        item_positions[current_unique_frame_index][2] - 10
            ...
            if unique_stepping:
                window.draw_text(
                    (-0.98, 0.9), str(current_unique_frame_index), (255, 255, 255), 0.2
                )
                window.draw_text(
                    (-0.98, 0.75),
                    str(unique_frames[current_unique_frame_index]),
                    (255, 255, 255),
                    0.2,
                )
            else:
                ...
                window.draw_text(
                    (-0.98, 0.75), str(frames[index]), (255, 255, 255), 0.2
                )
            ...
            if window.keybind("return") == 1 or (
                window.keybind("inventory") == 1
                and not menu.inventory_page.search_selected
            ):
