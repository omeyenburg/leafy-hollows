# -*- coding: utf-8 -*-
from game.world_generation import generate_world
...
class World:
    def __init__(
        ...
        self.chunks = (
            {}
        )  # {(chunk_x, chunk_y): numpy_array(16x16x4)} -> (block, plant, background, water_level)
        ...
        self.block_family: dict = {
            (name + "_flipped" if flipped else name): family
        ...
        self.block_name: dict = {
            (name + "_flipped" if flipped else name): index + flipped
            for name, (index, family, layer) in block_data.items()
            for flipped in range(2)
        }
        self.block_index: dict = {
            index + flipped: (name + "_flipped" if flipped else name)
            for name, (index, family, layer) in block_data.items()
            for flipped in range(2)
        }
        ...
        self.blocks_climbable: set = {
            self.block_name[name] for name in BLOCKS_CLIMBABLE
        }
    ...
    def update(self, window):
        ...
        # Update wind
        self.wind = (
            sin(window.time) * WORLD_WIND_STRENGTH
            + cos(window.time * 5) * WORLD_WIND_STRENGTH / 2
        )
        ...
        for y in geometry.shuffled_range(self.view_size[1] - 1):
            for x in geometry.shuffled_range(self.view_size[0] - 1):
                self.update_block(
                    window, self.loaded_blocks[0][0] + x, self.loaded_blocks[0][1] + y
                )
        ...
        if window.options["particles"]:
            ...
            if particle.spawn_possible(window, "small_leaf_particle"):
                x = random.randint(self.loaded_blocks[0][0], self.loaded_blocks[1][0])
                particle.spawn(
                    window, "small_leaf_particle", x, self.loaded_blocks[1][1]
                )
    ...
    def update_block_water(self, window, x, y):
        ...
        if not self.get_block(x, y - 1):
            ...
            absorbable_water = max(
                0, WORLD_WATER_PER_BLOCK - water_level_below + overflow_water
            )
        ...
        for _x, _y in ((x - 1, y), (x + 1, y)):
            if not (
                self.get_block(_x, _y)
                or self.get_block(_x, _y - 1)
                and self.get_water(_x, _y - 1) > 1
            ):
        ...
        self.set_water(
            x, y + 1, (water_level_target_above + emmitable_water) * water_side
        )
    ...
    def create_view(self, window):
        ...
        uncut_view = numpy.empty(
            (chunk_num_x * WORLD_CHUNK_SIZE, chunk_num_y * WORLD_CHUNK_SIZE, 4)
        )
        ...
        for chunk_delta_x, chunk_delta_y in numpy.ndindex((chunk_num_x, chunk_num_y)):
            ...
            if not (chunk_x, chunk_y) in self.chunks:
                self.chunks[(chunk_x, chunk_y)] = numpy.zeros(
                    (WORLD_CHUNK_SIZE, WORLD_CHUNK_SIZE, 4)
                )
                self.chunks[(chunk_x, chunk_y)][:, :, 0] = self.block_name["dirt_block"]

            uncut_view[
                chunk_delta_x
                * WORLD_CHUNK_SIZE : (chunk_delta_x + 1)
                * WORLD_CHUNK_SIZE,
                chunk_delta_y
                * WORLD_CHUNK_SIZE : (chunk_delta_y + 1)
                * WORLD_CHUNK_SIZE,
            ] = self.chunks[(chunk_x, chunk_y)]

        window.world_view = self.view = uncut_view[
            start_mod_x : start_mod_x + self.view_size[0],
            start_mod_y : start_mod_y + self.view_size[1],
        ]
