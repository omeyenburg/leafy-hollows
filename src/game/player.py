# -*- coding: utf-8 -*-
from game.baseentity import LivingEntity
...
class Player(LivingEntity):
    ...
    def draw(self, window: Window):
        ...
        if window.options["test.draw_hitboxes"]:
            rect = window.camera.map_coord(
                (self.rect.x, self.rect.y, self.rect.w, self.rect.h), from_world=True
            )
            window.draw_rect(rect[:2], rect[2:], (255, 0, 0, 100))

        # Draw player
        rect = window.camera.map_coord(
            (self.rect.x - 1 + self.rect.w / 2, self.rect.y, 2, 2), from_world=True
        )
        window.draw_image(
            "player_" + self.state,
            rect[:2],
            ...
            animation_offset=self.uuid,
        )

        if self.attack_time:
            if self.attack_time > 0 and self.state in (
                "idle",
                "walk",
                "sprint",
                "fall",
                ...
                window.draw_image(
                    "player_" + self.state, rect[:2], rect[2:], flip=(self.direction, 0)
                )
    ...
    def move_normal(self, world, window: Window):
        ...
        if window.keybind(
            "sprint"
        ):  # Keeps sprinting once key pressed / stops faster as long as pressed
        ...
        # Set current speed
        acceleration_speed = (
            max_speed * block_fricton * self.acceleration_time * (1 + block_fricton)
        )
        ...
        if not (self.block_below or self.block_right or self.block_left):
            ...
            if (
                PHYSICS_REALISTIC
                or PHYSICS_PREVENT_MOVEMENT_IN_AIR
                or not (key_right and key_left)
            ):
        ...
        if key_right and self.vel[0] < max_speed:
            if (
                self.vel[0] + acceleration_speed > max_speed
            ):  # Guaranteeing exact max speed
        ...
        # Auto jump
        if (
            window.options["auto jump"]
            and self.block_below
            and (bool(key_right) ^ bool(key_left))
            and (not window.keybind("jump"))
            and world.get_block(
                floor(
                    abs(
                        -self.rect.left * bool(key_left)
                        + self.rect.right * bool(key_right)
                        + 0.6
                    )
                ),
                round(self.rect.y),
        ...
        if window.keybind("jump") and (
            ...
            elif (not self.charge_crouch_jump) and (
                not self.state in ("crouch", "crawl", "crouch_jump")
            ):
                # Normal jump
                self.jump(window, 5)
        elif (
            self.block_below
            and self.charge_crouch_jump
            and self.state in ("crouch", "crawl")
        ):
            # Crouch jump
            self.jump(
                window,
                self.crouch_jump_strength
                * min(self.crouch_jump_max_charge, self.charge_crouch_jump),
            )  # 1/4 seconds to charge max crouch jump
    ...
    def move_climb(self, world, window: Window):
        ...
        """
        block_head = world.get_block(
            round(self.rect.x), round(self.rect.y + 0.8), layer=2
        )
        block_feet = world.get_block(
            round(self.rect.x), round(self.rect.y - 0.2), layer=2
        )
            ...
            if (window.keybind("left") or window.keybind("right")) and on_pole:
                if abs(self.vel[0]) < 1:
                    if window.keybind("right"):
                        self.direction = 0
                        self.vel[0] += (
                            10 * window.delta_time**0.2 * (1 + 0.5 * (self.vel[1] < 0))
                        )
                    else:
                        self.direction = 1
                        self.vel[0] -= (
                ...
                self.vel[0] = 0
                if (
                    world.get_block(round(self.rect.x), round(self.rect.y + 1), layer=2)
                    in world.blocks_climbable
                ):
    ...
    def animation_normal(self, world, window: Window):
        ...
        if self.vel[1] < -5:
            ...
            key_right
            and self.block_right
            and world.get_block(*wall_block_right)
            or key_left
            and self.block_left
            and world.get_block(*wall_block_left)
        ) and not self.block_below:
            # On Wall
            self.state = "climb"
            if self.vel[1] < 0:
                self.vel[1] /= 2
            ...
            if (
                key_right
                and world.get_block(*wall_block_right)
                and not world.get_block(wall_block_right[0], round(self.rect.y + 1.3))
                or key_left
                and world.get_block(*wall_block_left)
                and not world.get_block(wall_block_left[0], round(self.rect.y + 1.3))
            ):
            ...
            # Crouch jump
            if (abs(self.vel[0]) > 2 or self.state == "jump") and not (
                world.get_block(*wall_block_right) and world.get_block(*wall_block_left)
            ):
                self.state = "jump"
            else:
                self.state = "high_jump"
        ...
        if (
            self.block_below
            and abs(self.vel[0]) > 1
            and not (self.block_right or self.block_left)
        ):
            if self.state == "crouch":
                self.state = "crawl"
    ...
    def adjust_hitbox(self, world):
        ...
        if self.state in ("crouch", "crouch_jump", "crawl"):
            ...
            if self.get_collision(world):
                ...
                self.state = {"crouch_jump": "fall", "crawl": "walk"}.get(
                    self.state, "idle"
                )
        else:
    ...
    def mouse_inputs(self, world, window: Window):
        ...
        # Mouse pull
        if (
            window.mouse_buttons[0] == 1 and window.options["test.player_leap"]
        ):  # Left click: pull player to mouse
            ...
            def mouse_pull(strenght):
                ...
                force = (
                    min(dist(self.rect.center, mouse_pos), 3)
                    / window.delta_time
                    * strenght
                )
            ...
            mouse_pull(
                200
            )  # constant activation balances out gravity --> usable as rope
        ...
        if window.mouse_wheel[3] and window.options["test.scroll_zoom"]:
            window.camera.zoom(
                window.camera.resolution_goal + window.mouse_wheel[3] * 0.05, 1
            )
        ...
        if window.mouse_buttons[2] == 1 and window.options["test.place_water"]:
            ...
            world.set_water(
                floor(mouse_pos[0]), floor(mouse_pos[1]), water_level + 1000
            )
    ...
    def obtain_weapon_drop(self, window, entity):
        ...
        if soul_drain_level:
            weapon_heal = (
                soul_drain_level
                * ATTRIBUTE_BASE_MODIFIERS["soul drain"]
                * 0.01
                * self.max_health
            )
        ...
        drop_chance = (
            entity.item_drop_chance
            * (weapon_looting + 1)
            * ATTRIBUTE_BASE_MODIFIERS["looting"]
            * 0.01
        )
        ...
        if random.random() < drop_chance or len(self.inventory.weapons) <= 3:
            ...
            if self.inventory.arrows < self.inventory.max_arrows:
                ...
                if (
                    arrow_drop < 0.15
                    or arrow_drop < 0.3
                    and len(self.inventory.weapons) <= 5
                ):
                    arrow_num = random.randint(1, 2) * 8
                    previous_arrows = self.inventory.arrows
                    self.inventory.arrows = min(
                        self.inventory.arrows + arrow_num, self.inventory.max_arrows
                    )
                    self.recent_drop = [
                        2,
                        "arrows",
                        self.inventory.arrows - previous_arrows,
                    ]
                    return
            ...
            if random.random() < 0.01:
                ...
                weapon = random.choice((Stick, Sword, Axe, Pickaxe, Bow))(
                    1 + weapon_looting // 3
                )
