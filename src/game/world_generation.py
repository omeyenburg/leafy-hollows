# -*- coding: utf-8 -*-
from utility.noise_functions import snoise2
...
def generate_world(world, window):
    ...
    """
    world.seed: float = (
        random.randint(-(10**6), 10**6) + e
    )  # Float between -10^6 and 10^6
        ...
        if next_special:
            ...
            if cave_type < 0.6:
                ...
                if structure_index == len(structures):
                    structure_names = random.sample(
                        list(structures.keys()), k=len(structures)
                    )
                    structure_index = 0
                ...
                cave.interpolated(
                    world,
                    position,
                    end_angle=structure_data["generation"]["entrance_angle"],
                    end_radius=structure_data["generation"]["entrance_size"] / 2,
                )

                generated_structure = (
                    round(
                        position[0] - structure_data["generation"]["entrance_coord"][0]
                    ),
                    round(
                        position[1] - structure_data["generation"]["entrance_coord"][1]
                    ),
                ...
                for dx, dy in numpy.ndindex(generated_structure[2].shape[:2]):
                    world.set_block(
                        generated_structure[0] + dx,
                        generated_structure[1] + dy,
                        world.block_name["dirt_block"],
                    )

                position[0] += (
                    structure_data["generation"]["exit_coord"][0]
                    - structure_data["generation"]["entrance_coord"][0]
                )
                position[1] += (
                    structure_data["generation"]["exit_coord"][1]
                    - structure_data["generation"]["entrance_coord"][1]
                )

                cave.interpolated(
                    world,
                    position,
                    start_angle=structure_data["generation"]["exit_angle"],
                    start_radius=structure_data["generation"]["exit_size"] / 2,
                )
    ...
    blocks_ground, blocks_ceiling, blocks_wall_right, blocks_wall_left = (
        find_edge_blocks(world)
    )
    ...
    # Generate foliage
    generate_foliage(
        world, blocks_ground, blocks_ceiling, blocks_wall_right, blocks_wall_left
    )

    # Spawn enemies
    window.loading_progress[:2] = "Spawing enemies", 11
    spawn_blocks = sorted(
        random.sample(list(blocks_ground), k=int(0.1 * len(blocks_ground)))
    )
    last_bat = 0
    ...
    for coord in spawn_blocks:
        if (
            coord[0] < 30
            or coord[1] > -500
            or world.get_block(coord[0], coord[1] + 1)
            or coord[0] > last_enemy_x
            or world.get_water(coord[0], coord[1])
        ):
            continue
        if coord[0] < 100:
...
def generate_foliage(
    ...
    blocks_ground = random.sample(
        list(blocks_ground), k=int(WORLD_VEGETATION_FLOOR_DENSITY * len(blocks_ground))
    )
    blocks_ceiling = random.choices(
        list(blocks_ceiling),
        k=int(WORLD_VEGETATION_CEILING_DENSITY * len(blocks_ceiling)),
    )
    blocks_wall_right = random.choices(
        list(blocks_wall_right),
        k=int(WORLD_VEGETATION_WALL_DENSITY * len(blocks_wall_right)),
    )
    blocks_wall_left = random.choices(
        list(blocks_wall_left),
        k=int(WORLD_VEGETATION_WALL_DENSITY * len(blocks_wall_left)),
    )
...
def get_decoration_block_type(world, x, y):
    ...
    # Exit early
    if (
        (block_below and block_above)
        or (0 < water_level < 700)
        or (block_below != world.block_name["grass_block"] and random.random() > 0.4)
    ):
        return [None]
    ...
    decoration_list = list(
        filter(
            lambda name: (
                any(
                    [
                        selected in block_comparison
                        for selected in world.block_generation_properties[name]
                        .get("on", "any")
                        .split("|")
                    ]
                )
                and side == world.block_generation_properties[name].get("side", "above")
                and (
                    bool(water_level)
                    == world.block_generation_properties[name].get("water", False)
                    or "any"
                    == world.block_generation_properties[name].get("water", False)
                )
                and corner
                == world.block_generation_properties[name].get("corner", False)
            ),
    ...
    decoration_block = random.choices(
        decoration_list,
        weights=[
            world.block_generation_properties[name].get("weight", 1)
...
def generate_decoration_block(world, x, y, decoration_block, flipped, side):
    expansion_length = int(
        sqrt(
            random.random()
            * (
                world.block_generation_properties[decoration_block].get(
                    "expansion_length", 1
                )
                - 1
            )
    ...
    expansion_direction = {"up": pi / 2, "down": -pi / 2, "left": pi, "right": 0}.get(
        world.block_generation_properties[decoration_block].get(
    ...
    if size == (1, 1):
        ...
        for dx, dy in numpy.ndindex(size):
            if any(
                world.get_block(
                    coord[0] + dx, coord[1] - dy, layer=slice(3), default=[1]
                )
            ):
                return
...
def generate_poles(world, poles, blocks_ground, blocks_ceiling):
    ...
    for x in poles:
        ...
        for x_offest in range(-2, 3):
            if not (
                (x + x_offest) in blocks_ground and (x + x_offest) in blocks_ceiling
            ):
                continue
...
def flatten_edges(world):
    ...
    for x, y in world_copy.iterate():
        block_types = [
            world_copy.get_block(x + dx, y + dy, layer=0, default=1)
            for dx in range(-1, 2)
            for dy in range(-1, 2)
        ]
