...
class PhysicsObject:
    ...
    def apply_force(
        self, force: float, angle: float, delta_time: float, is_radians: bool = False
    ):
        """
    ...
    def apply_velocity(self, world, delta_time: float):
        ...
        self.rect.x += min(
            PHYSICS_MAX_MOVE_DISTANCE,
            max(-PHYSICS_MAX_MOVE_DISTANCE, self.vel[0] * delta_time),
        )
        ...
        self.rect.y += min(
            PHYSICS_MAX_MOVE_DISTANCE,
            max(-PHYSICS_MAX_MOVE_DISTANCE, self.vel[1] * delta_time),
        )
    ...
    def x_collide(self, world, delta_time):
        ...
        for x in range(
            floor(round(self.rect.left, 5)), ceil(round(self.rect.right, 5))
        ):
            # for y in range(floor(round(self.rect.y, 5)), ceil(round(self.rect.y + self.rect.h, 5))):
            for y in range(
                floor(round(self.rect.top, 5)), ceil(round(self.rect.bottom, 5))
            ):
                if world.get_block(x, y):
                    ...
                    if not (
                        ...
                        or self.vel[1] <= 0
                        or abs(self.vel[0]) < 1
                        or self.rect.y - last_y > 0.5
                        or collision
                        or self.get_collision(world)
                    ):
                        self.vel[1] *= 0.8
                        continue
    ...
    def update(self, world, delta_time):
        ...
        if block_fricton:
            self.apply_force_horizontal(
                -self.vel[0] * self.mass * block_fricton * 0.1, 1
            )
        ...
        for other in world.loaded_entities:
            ...
            angle = atan2(
                (self.rect.centery - other.rect.centery) * 0.5,
                self.rect.centerx - other.rect.centerx + random.random() * 0.1,
            )
