...
def load(block_name):
    ...
    for path in structure_paths:
        ...
        entrance_angle = data["generation"]["entrance_angle"] * pi / 180
        exit_coord = data["generation"]["exit_coord"]
        exit_angle = data["generation"]["exit_angle"] * pi / 180
        data["generation"]["entrance_angle"] = entrance_angle
        data["generation"]["exit_angle"] = exit_angle

        entrance_ceiling = find_cave_wall(
            array, entrance_coord, entrance_angle + pi / 2
        )
        entrance_floor = find_cave_wall(array, entrance_coord, entrance_angle - pi / 2)
        entrance_size = sqrt(
            (entrance_ceiling[0] - entrance_floor[0]) ** 2
            + (entrance_ceiling[1] - entrance_floor[1]) ** 2
        )
        entrance_coord = (
            (entrance_ceiling[0] + entrance_floor[0]) / 2,
            (entrance_ceiling[1] + entrance_floor[1]) / 2,
        ...
        exit_floor = find_cave_wall(array, exit_coord, exit_angle - pi / 2)
        exit_size = (
            sqrt(
                (exit_ceiling[0] - exit_floor[0]) ** 2
                + (exit_ceiling[1] - exit_floor[1]) ** 2
            )
            - 2
        )
        exit_coord = (
...
def find_cave_wall(array, start, angle):
    ...
    while 0 <= x < width and 0 <= y < height and array[x, y, 0] == 0:
        x = round(start[0] + cos(angle) * i)
        y = round(start[1] + sin(angle) * i)
        i += 0.1
