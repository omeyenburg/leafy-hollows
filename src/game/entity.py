...
class GreenSlime(LivingEntity):
    def __init__(self, spawn_pos: [float], health=5, hit_damage=1):
        ...
        self.hit_damage = min(
            hit_damage + 2, round(hit_damage * (1 + spawn_pos[0] / 400 / hit_damage))
        )
    ...
    def draw(self, window: Window):
        ...
        rect = window.camera.map_coord(
            (self.rect.x - 0.5 + self.rect.w / 2, self.rect.y, 1, 1), from_world=True
        )
        window.draw_image(
            "_".join((self.slime_variant, self.image, self.state)),
            rect[:2],
            rect[2:],
    ...
    def update(self, world, window: Window):
        ...
        if self.block_below and not self.stunned:
            if self.hit_ground == 0:
                ...
                particle.spawn(
                    window,
                    self.slime_variant + "_slime_particle",
                    self.rect.centerx,
                    self.rect.top,
                )
            ...
            self.hit_ground = 0
            self.vel[0] += (
                10 * (-self.direction + 0.5) * self.jump_strength * window.delta_time
            )
        ...
        if (
            self.attack_cooldown < 0
            and self.vel[1] < 0
            and self.rect.collide_rect(world.player.rect)
        ):
    ...
    def death(self, window):
        particle.spawn(
            window, self.slime_variant + "_slime_particle", *self.rect.center
        )
...
class Goblin(LivingEntity):
    ...
    def update(self, world, window: Window):
        ...
        if world.player.rect.centerx < self.rect.centerx:
            self.vel[0] = -speed
            self.direction = 1
            side_block = world.get_block(
                floor(self.rect.left - 0.6), round(self.rect.y)
            ) and not world.get_block(
                floor(self.rect.left - 0.6), round(self.rect.y + 1)
            )
        else:
            self.vel[0] = speed
            self.direction = 0
            side_block = world.get_block(
                floor(self.rect.right + 0.6), round(self.rect.y)
            ) and not world.get_block(
                floor(self.rect.right + 0.6), round(self.rect.y + 1)
            )
        ...
        if (
            self.underWater
            and world.player.rect.centery > self.rect.centery
            and self.vel[1] < 2.5
        ):
        ...
        sound.play(
            ...
            x=(self.rect.x - world.player.rect.x) / 5,
        )
        ...
        if self.rect.collide_rect(world.player.rect) or holding_bow:
            ...
            if self.prepare_attack < 0:
                angle = atan2(
                    world.player.rect.centery - self.rect.centery,
                    world.player.rect.centerx - self.rect.centerx,
                )
                if holding_bow:
    ...
    def move(self, world, window: Window):  # pathfinding implementation
        def pathfind() -> list[int, int]:
            ...
            if (
                self.path is None
                or len(self.path) == 0
                or dist(self.path[0], world.player.rect.center) > 3
            ):
                self.path_search_delay += window.delta_time
                if (
                    self.path_search_delay > PATH_FIND_DELAY
                ):  # Recalculate path when player moved too far
                    self.path_search_delay = 0

                    grid: list[list[int, int]] = numpy.swapaxes(
                        world.view[:, :, 0], 0, 1
                    )
                    ...
                    start_pos = [
                        int(world.player.rect.center[0] - world.loaded_blocks[0][0]),
                        int(world.player.rect.center[1] - world.loaded_blocks[0][1]),
                    ]
                    end_pos = [
                        int(self.rect.center[0] - world.loaded_blocks[0][0]),
                        int(self.rect.center[1] - world.loaded_blocks[0][1]),
                    ]
                    result = a_star(
                        grid=grid, start_pos=start_pos, end_pos=end_pos, full_path=True
                    )
                    if result is None:
                        return None

                    self.path = [
                        (x + world.loaded_blocks[0][0], y + world.loaded_blocks[0][1])
                        for x, y in result
                    ]
            ...
            if window.options["test.draw_pathfinding"]:
                window.draw_block_highlight(
                    self.path[0][0], self.path[0][1], (255, 0, 255, 100)
                )
                window.draw_block_highlight(
                    self.path[-1][0], self.path[-1][1], (0, 255, 0, 100)
                )
        ...
        if (
            dist(self.rect.center, world.player.rect.center) > 3
        ):  # Find path for longer distances
            next_pos = pathfind()

            if not next_pos:
                next_pos = (
                    world.player.rect.centerx,
                    world.player.rect.y + self.prepare_attack * 2,
                )
            else:
            ...
            next_pos = (
                world.player.rect.centerx,
                world.player.rect.y + world.player.rect.h + self.prepare_attack * 2,
            )

        speed_x = min(
            self.max_speed, 0.1 + (world.player.rect.centerx - self.rect.centerx) ** 2
        )
        ...
        speed_y = min(
            self.max_speed,
            0.1 + abs(world.player.rect.y + world.player.rect.h - self.rect.centery),
        )
