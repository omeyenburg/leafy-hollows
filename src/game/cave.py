# -*- coding: utf-8 -*-
from utility.noise_functions import pnoise1, snoise2
from utility.const import *


def generate_points_segment(
    position: [float], length, start_angle: float, deviation: float
):
    angle = start_angle
    points = set()
...
def line_cave(world, position, length, angle, deviation, radius):
    ...
    for x, y in points:
        ...
        for delta_x in range(
            -p_radius - WORLD_GENERATION_CAVE_BORDER_PADDING,
            p_radius + WORLD_GENERATION_CAVE_BORDER_PADDING + 1,
        ):
...
def intro(world, window, position):
    ...
    for i in range(length):
        position[0] = (
            pnoise1(i * 16 + world.seed, octaves=2, repeat=INTRO_REPEAT * 16)
            * deviation
        )
        position[1] = -i
        points.add(tuple(position))
    ...
    for x, y in points:
        ...
        for delta_x in range(
            -radius - WORLD_GENERATION_CAVE_BORDER_PADDING,
            radius + WORLD_GENERATION_CAVE_BORDER_PADDING + 1,
        ):
            for delta_y in range(
                -radius - WORLD_GENERATION_CAVE_BORDER_PADDING,
                radius + WORLD_GENERATION_CAVE_BORDER_PADDING + 1,
...
def horizontal(world, position):
    ...
    line_cave(
        ...
        angle,
        deviation,
        WORLD_GENERATION_HORIZONTAL_CAVE_RADIUS,
    )
...
def interpolated(
    ...
    if start_angle is None:
        start_angle = (
            snoise2(position[0] / 100 + world.seed, world.seed, octaves=4) * 0.6
        )
    if end_angle is None:
        end_angle = (
            snoise2(
                (
                    position[0]
                    + WORLD_GENERATION_INTERPOLATION_LENGTH * WORLD_GENERATION_STEP_SIZE
                )
                / 100
                + world.seed,
                world.seed,
    ...
    if start_radius is None:
        start_radius = int(
            pnoise1(sum(position) / 2 + 100, octaves=3) * 2
            + WORLD_GENERATION_HORIZONTAL_CAVE_RADIUS
        )
