# -*- coding: utf-8 -*-
from utility.noise_functions import pnoise1
...
class Inventory:
    ...
    def update(self, window, menu, world):
        if menu.inventory_page.fusing != 0:
            menu.inventory_page.fusing = min(
                int(menu.inventory_page.fusing >= 0),
    ...
    def update_inventory(self, window, menu, world):
        ...
        window.draw_image(
            "search_icon", (-0.98, -0.98), (0.09, 0.09 / window.height * window.width)
        )

        if 1 in window.mouse_buttons:
            menu.inventory_page.search_selected = (
                window.mouse_pos[0] < 0
                and window.mouse_pos[1] / window.height * 2 < -0.8
            )
        ...
        if menu.inventory_page.search_selected:
            ...
            if "\x08" in window.unicode:
                if search_text_length:
                    menu.inventory_page.search_text = menu.inventory_page.search_text[
        ...
        if menu.inventory_page.search_text:
            search_text_width = window.draw_text(
                (-0.85, -0.9), menu.inventory_page.search_text, (255, 255, 255), 0.2
            )[0]
        else:
        ...
        if menu.inventory_page.search_selected and window.time % 1 < 0.5:
            window.draw_rect(
                (-0.85 + search_text_width, -0.97), (0.005, 0.14), (255, 255, 255)
            )

        # Filter weapon list with search parameters
        if menu.inventory_page.sort_key == "Level":
            weapon_sort_function = (
                lambda i: (not i is world.player.holding) * 99999999
                + (not i.uuid in world.player.inventory.marked_weapons) * 9999999
                - max(i.attributes.values())
                - 0.1 * min(i.attributes.values())
            )
        elif menu.inventory_page.sort_key == "Type":
            weapon_sort_function = lambda i: (
                not i.uuid in world.player.inventory.marked_weapons
            ) * 9999999 + sum(map(lambda j: ord(j), i.image))
        ...
        inventory = sorted(
            filter(
                lambda i: search_text in translate(window.options["language"], i.image)
                or any(
                    [
                        search_text
                        in translate(window.options["language"], attribute).lower()
                        or search_text
                        in translate(
        ...
        for i, key in enumerate(("Level", "Type", "Age")):
            ...
            rect = Rect(
                -0.95,
                -i * 0.2 - 0.05 / window.height * window.width - 0.2,
                0.5,
                0.1 / window.height * window.width,
            )
            if 1 in window.mouse_buttons and rect.collide_point(
                (
                    window.mouse_pos[0] / window.width * 2,
                    window.mouse_pos[1] / window.height * 2,
                )
            ):
        ...
        # Get mouse scroll
        scroll_speed = -20
        window.mouse_wheel[1] = max(
            min(window.mouse_wheel[1], 0), len(inventory) * scroll_speed - scroll_speed
        )
        scroll_position = window.mouse_wheel[1] / scroll_speed
        if not window.mouse_wheel[3]:
            window.mouse_wheel[1] += (
                (round(scroll_position) - scroll_position) * scroll_speed * 0.02
            )
        ...
        for i, weapon in enumerate(inventory):
            weapon_size = Vec(0.4, 0.4 / window.height * window.width) * max(
                0.5, 1 / ((i - scroll_position) ** 2 + 1)
            )
            weapon_y -= weapon_size[1]
            weapon_positions.append(weapon_y)

        distance = abs(floor(scroll_position) - scroll_position)
        weapon_center_y = (1 - distance) * weapon_positions[
            floor(scroll_position)
        ] + distance * weapon_positions[ceil(scroll_position)]

        # Draw weapon list
        for i, weapon in enumerate(inventory):
            weapon_size = Vec(0.4, 0.4 / window.height * window.width) * max(
                0, 1 / ((i - scroll_position) ** 4 + 1)
            )
            weapon_pos = Vec(
                -0.2 - weapon_size[0] / 2, weapon_positions[i] - weapon_center_y
            )
            ...
            star_rect = Rect(
                -0.2 - weapon_size[0] * 0.55,
                weapon_positions[i] - weapon_center_y + weapon_size[1] * 0.9,
                *(weapon_size * 0.2),
            )

            if (
                1 in window.mouse_buttons
                and weapon_rect.collide_point(
                    (
                        window.mouse_pos[0] / window.width * 2,
                        window.mouse_pos[1] / window.height * 2,
                    )
                )
                and abs(round(scroll_position) - i) == 1
            ):
                if scroll_position < i:
                    window.mouse_wheel[1] = (
                        ceil(round(window.mouse_wheel[1] / scroll_speed) + 0.1) - 0.3
                    ) * scroll_speed
                else:
                    window.mouse_wheel[1] = (
                        floor(round(window.mouse_wheel[1] / scroll_speed) - 0.1) + 0.3
                    ) * scroll_speed
                sound.play(window, "click")

            if (
                1 in window.mouse_buttons
                and star_rect.collide_point(
                    (
                        window.mouse_pos[0] / window.width * 2,
                        window.mouse_pos[1] / window.height * 2,
                    )
                )
                and round(scroll_position) == i
            ):
            ...
            if i == round(scroll_position):
                radius = (
                    weapon_size[0] * 0.5 + pnoise1(window.time * 0.1, octaves=2) * 0.1
                )
        ...
        if weapon is world.player.holding:
            window.draw_text(
                (name_width + name_width / len(name), 0.8),
                "(equipped)",
                (255, 255, 255),
                0.3,
            )[0]
        ...
        for i, stat in enumerate(("damage", "attack_speed", "range", "crit_chance")):
            ...
            stat_size = window.draw_text(
                (0, -i * 0.1 + 0.6),
                f"{stat_name}: {weapon_base_stats[i]}",
                (164, 221, 219),
                0.17,
            )
        ...
        for i, (attribute, level) in enumerate(weapon.attributes.items()):
            ...
            description = (
                translate(window.options["language"], ATTRIBUTE_DESCRIPTIONS[attribute])
                % modifier
            )
            window.draw_text(
                (0, description_y),
                f"{name} {INT_TO_ROMAN.get(level, level)}: {description}",
                (223, 132, 165),
                0.17,
                wrap=1,
            )

        matching_items_text = translate(
            window.options["language"],
            "%s matching item" + "s" * int(len(inventory) != 1),
        ) % str(len(inventory))
        ...
        for i, key in enumerate(("Equip", "Fuse", "Destroy")):
            rect = Rect(
                -0.95,
                -i * 0.2 - 0.05 / window.height * window.width + 0.7,
                0.5,
                0.1 / window.height * window.width,
            )
            sort_image = "button_dark_unselected"

            if rect.collide_point(
                (
                    window.mouse_pos[0] / window.width * 2,
                    window.mouse_pos[1] / window.height * 2,
                )
            ...
            action_width = window.draw_text(
                (-0.9, -i * 0.2 + 0.7), button_text, (255, 255, 255), 0.17
            )[0]
            if key == "Destroy":
                window.draw_text(
                    (-0.85 + action_width, -i * 0.2 + 0.7),
                    "+" + str(destroy_health_gain),
                    (255, 255, 255),
                    0.17,
                )
            window.draw_image(
                button_image,
                (
                    -0.53 - button_image_size * 0.2,
                    -i * 0.2 + 0.7 - button_image_size / 2,
                ),
                (button_image_size * window.height / window.width, button_image_size),
            )

    def update_fuse(self, window, menu, world):
        # Main fusion weapon
        weapon = menu.inventory_page.fuse_item
        weapon_size = Vec(1, 1 / window.height * window.width) * (
            0.4
            - 0.1
            * (
                max(0, min(menu.inventory_page.fusing, 0.5))
                + 0.5 * (menu.inventory_page.fusing < 0)
            )
        )
        weapon_pos = Vec(
            -0.2
        ...
        window.draw_text(
            (-0.93, -0.98 + 0.1 / window.height * window.width / 2),
            "Back",
            (255, 255, 255),
            0.17,
        )
        ...
        for i, (attribute, level) in enumerate(weapon.attributes.items()):
            ...
            window.draw_text(
                (-0.6, description_y),
                f"{attribute_name} {INT_TO_ROMAN[level]}",
                (223, 132, 165),
                0.17,
                wrap=1,
            )
        ...
        window.draw_image(
            "search_icon", (0.02, -0.98), (0.09, 0.09 / window.height * window.width)
        )
        ...
        if menu.inventory_page.search_text:
            search_text_width = window.draw_text(
                (0.15, -0.9), menu.inventory_page.search_text, (255, 255, 255), 0.2
            )[0]
        else:
        ...
        if menu.inventory_page.search_selected and window.time % 1 < 0.5:
            window.draw_rect(
                (0.15 + search_text_width, -0.97), (0.005, 0.14), (255, 255, 255)
            )
        ...
        inventory = sorted(
            filter(
                lambda i: (
                    search_text in translate(window.options["language"], i.image)
                    or any(
                        [
                            search_text
                            in translate(window.options["language"], attribute).lower()
                ...
                and not i is menu.inventory_page.fuse_item
                and any(
                    [
        ...
        if menu.inventory_page.secondary_fuse_item is None:
            ...
            scroll_speed = -20
            window.mouse_wheel[1] = max(
                min(window.mouse_wheel[1], 0),
                len(inventory) * scroll_speed - scroll_speed,
            )
            scroll_position = window.mouse_wheel[1] / scroll_speed
            if not window.mouse_wheel[3]:
                window.mouse_wheel[1] += (
            ...
            # Draw weapon list
            for i, weapon in enumerate(inventory):
                weapon_size = Vec(0.3, 0.3 / window.height * window.width) * max(
                    0, 1 / ((i - scroll_position) ** 4 + 1)
                )
                weapon_pos = Vec(
                    0.8 - weapon_size[0] / 2, weapon_positions[i] - weapon_center_y
                )
                ...
                if (
                    ...
                    if scroll_position < i:
                        window.mouse_wheel[1] = (
                            ceil(round(window.mouse_wheel[1] / scroll_speed) + 0.1)
                            - 0.3
                        ) * scroll_speed
                    else:
                        window.mouse_wheel[1] = (
                            floor(round(window.mouse_wheel[1] / scroll_speed) - 0.1)
                            + 0.3
                        ) * scroll_speed
                ...
                if i == round(scroll_position):
                    radius = (
                        weapon_size[0] * 0.5
                        + pnoise1(window.time * 0.1, octaves=2) * 0.1
                    )
                    if weapon is world.player.holding:
                        color = (168, 202, 88, 50)
                    else:
                ...
                window.draw_image(
                    weapon.image, weapon_pos, weapon_size, angle=weapon.angle
                )
            ...
            window.draw_image(
                secondary_weapon.image,
                weapon_pos,
                weapon_size,
                angle=secondary_weapon.angle,
            )
        ...
        fusion_attributes = {
            attribute: max(
                level + (level == secondary_weapon.attributes.get(attribute, 0)),
                secondary_weapon.attributes.get(attribute, 0),
            )
        ...
        rect = Rect(
            -0.25,
            -0.1 / window.height * window.width / 2,
            0.5,
            0.1 / window.height * window.width,
        )
        fuse_button_image = "button_dark_unselected"
        ...
        if menu.inventory_page.fusing > 0:
            ...
            fuse_bar_image = "fuse_bar_" + chr(
                ord("a") + round(6 + menu.inventory_page.fusing * 4)
            )

        window.draw_image(
            fuse_bar_image, (-0.5, -0.4), (0.3, 0.3 / window.height * window.width)
        )
        window.draw_image(
            fuse_bar_image,
            (0.2, -0.4),
            (0.3, 0.3 / window.height * window.width),
            flip=(1, 0),
        )
