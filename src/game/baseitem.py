...
class BaseItem:
    ...
    def apply_attributes(self, window, attacker, target):
        ...
        if vampire_level:
            weapon_heal = (
                vampire_level
                * ATTRIBUTE_BASE_MODIFIERS["vampire"]
                * 0.01
                * attacker.max_health
            )
        ...
        if paralysis_level:
            weapon_paralysis = (
                paralysis_level * ATTRIBUTE_BASE_MODIFIERS["paralysis"] * 0.01
            )
    ...
    def get_weapon_stat_increase(self, world):
        ...
        if warrior_level:
            number_enemies = len(
                list(filter(lambda i: i.type == "enemy", world.loaded_entities))
            )
            warrior = (
                warrior_level
                * ATTRIBUTE_BASE_MODIFIERS["warrior"]
                * 0.01
                * number_enemies
            )
            damage *= 1 + warrior
...
class MeleeWeapon(BaseItem):
    ...
    def attack(self, window, world, attacker, angle, arg_range=None):
        ...
        damage, attack_speed, weapon_range, crit_chance = self.get_weapon_stat_increase(
            world
        )
        ...
        for entity in world.loaded_entities:
            ...
            if entity_distance < 1:
                ...
                continue
            entity_angle = atan2(
                entity.rect.centery - attacker.rect.centery,
                entity.rect.centerx - attacker.rect.centerx,
            )
        ...
        if not targets:
            sound.play(
                window, "sword_swing", x=(world.player.rect.x - attacker.rect.x) / 5
            )
            return
...
class RangedWeapon(BaseItem):
    ...
    def attack(self, window, world, attacker, angle):
        ...
        world.add_entity(
            Arrow(
                attacker.rect.center,
                speed=weapon_range * 10,
                angle=angle,
                owner=attacker,
            )
        )
